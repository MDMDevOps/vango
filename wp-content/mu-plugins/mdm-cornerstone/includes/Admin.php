...
class Admin extends Framework {
	...
	public function addActions() {
		...
		$this->subscriber->addAction( 'admin_enqueue_scripts', [$this, 'enqueue_scripts'] );
		$this->subscriber->addAction( 'admin_enqueue_scripts', [$this, 'enqueue_styles'] );
		$this->subscriber->addAction( 'menu_order', [$this, 'reorder_admin_menu'], 99999 );
		$this->subscriber->addAction( 'custom_menu_order', '__return_true' );
		$this->subscriber->addAction( 'admin_menu', [$this, 'rename_menu_items'] );
		$this->subscriber->addAction( 'jetpack_just_in_time_msgs', '__return_false' );
		$this->subscriber->addAction( 'wp_dashboard_setup', [$this, 'remove_unwanted_dashboard_widgets'], 99 );
		$this->subscriber->addAction( 'activate_bb-plugin/fl-builder.php', [$this, 'set_flbuilder_license'] );
		$this->subscriber->addAction( 'init', [$this, 'init_customizer'] );
	}
	/**
	 * Register filters
	 *
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script( sprintf( '%s_admin', 'mdm_cornerstone' ), self::url( 'assets/js/admin.js' ), array( 'jquery' ), MDM_CORNERSTONE_VERSION, true );
		// Localize public script
		wp_localize_script( sprintf( '%s_admin', 'mdm_cornerstone' ), 'mdm_cornerstone', array( 'wpajaxurl' => admin_url( 'admin-ajax.php') ) );
	}

	public function enqueue_styles() {
		// Register admin styles
		wp_enqueue_style( sprintf( '%s_admin', 'mdm_cornerstone' ), self::url( 'assets/css/admin.css' ), array(), MDM_CORNERSTONE_VERSION, 'all' );
	}
